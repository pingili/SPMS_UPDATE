...
BEGIN
	...
	SELECT A.AHID, A.AHCODE, A.AHNAME,AH.AHName AS BankName, B.AccountNumber,B.BranchName ,TDATE,VoucherNumber, ABS(A.CrAmount) AS CrAmount, ABS(A.DrAmount) DrAmount,AH.OpeningBalance, (SELECT ABS(SUM(BAL)) FROM ENTRIES B WHERE B.ROWID <= A.ROWID) AS Balance
